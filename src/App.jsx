...
async function exportVideo({
    ...
    outCanvas.width = safeWidth;
    outCanvas.height = safeHeight;
    ...
    try {
        for (let i = 0; i < totalFrames; i++) {
            ...
            // ✅ 출력 해상도용 캔버스(항상 safeWidth x safeHeight)
            // ✅ React 렌더 반영 (setT 후 실제 DOM이 바뀐 다음 캡처)
            await new Promise((r) => requestAnimationFrame(() => requestAnimationFrame(r)));

            // 1) element를 실제 크기로 캡처 (여기서는 element 크기 그대로 canvas가 나옴)
            const rect = element.getBoundingClientRect();
            const baseScale = safeWidth / Math.max(1, rect.width);
            // 너무 과하게 키우면 느려지니 상한 걸기(4K는 3까지, 그 외 2.5까지 추천)
            const capScale = Math.min(is4K ? 3 : 2.5, Math.max(1, baseScale));

            const shot = await html2canvas(element, {
                scale: capScale,
                backgroundColor: null, // ✅ 투명으로 뽑고, 우리가 outCanvas에서 배경 채움
                useCORS: true,
                logging: false,
            });

            // 2) outCanvas 배경 채우기 (검은 여백 방지)
            const bg = getComputedStyle(element).backgroundColor;
            const bgColor =
                bg && bg !== "rgba(0, 0, 0, 0)" && bg !== "transparent" ? bg : "#f3f4f6";

            outCtx.clearRect(0, 0, safeWidth, safeHeight);
            outCtx.fillStyle = bgColor;
            outCtx.fillRect(0, 0, safeWidth, safeHeight);

            // 3) shot(작은 캔버스)을 outCanvas(출력 해상도)에 "비율 유지(contain)로 확대 + 가운데 배치"
            const s = Math.min(safeWidth / shot.width, safeHeight / shot.height);
            const dw = Math.round(shot.width * s);
            const dh = Math.round(shot.height * s);
            const dx = Math.round((safeWidth - dw) / 2);
            const dy = Math.round((safeHeight - dh) / 2);

            outCtx.drawImage(shot, dx, dy, dw, dh);

            // 4) outCanvas 자체를 VideoFrame으로 인코딩 (항상 safeWidth x safeHeight라 패딩/좌상단 문제 사라짐)
            const frame = new VideoFrame(outCanvas, { timestamp: (i * 1_000_000) / fps });
...
function ChartPreview({
    ...
    return (
        <div className="chartWrap">
            <div className="chartCanvas flourishCanvas" ref={captureRef}>
                {title ? (
                    <div
                ...
                <aside className="sidebar">
                    ...
                    <Section title="차트 제목">
                        ...
                        <FieldRow label="정렬 위치">
                            <div className="seg">
                                <button type="button" className={`segBtn ${titleAlign === "left" ? "active" : ""}`} onClick={() => setTitleAlign("left")}>왼쪽</button>
                                <button type="button" className={`segBtn ${titleAlign === "center" ? "active" : ""}`} onClick={() => setTitleAlign("center")}>가운데</button>
                                <button type="button" className={`segBtn ${titleAlign === "right" ? "active" : ""}`} onClick={() => setTitleAlign("right")}>오른쪽</button>
                            </div>
                        </FieldRow>
                    </Section>
                    ...
                    {tab === "data" ? (
                        <>
                            <div className="sidebarBlock">
                                <Section title="데이터 관리" defaultOpen={true}>
                                    <div style={{ display: "flex", flexDirection: "column", gap: "10px", marginTop: "10px", padding: "0 4px" }}>
                                        <button className="primaryBtn" onClick={handleAddColumn} style={{ padding: '8px 12px', cursor: 'pointer', width: '100%', textAlign: 'center' }}>+ 열 추가하기</button>
                                        <button className="ghostBtn" onClick={handleDeleteColumn} style={{ padding: '8px 12px', cursor: 'pointer', width: '100%', textAlign: 'center', color: '#dc2626', borderColor: '#fca5a5' }}>- 열 삭제하기</button>
                                        <div className="hintText" style={{ fontSize: 13, color: '#666', lineHeight: '1.4', marginTop: '8px' }}>💡 Tip: 'Image' 열을 추가하고 이미지 주소를 입력해보세요.</div>
                                    </div>
                                </Section>
                            </div>
                            ...
                            <div className="sidebarBlock">
                                <label className="uploadBtn sidebarUpload">
                                    데이터 업로드
                                    <input type="file" accept=".xlsx,.xls,.csv" onChange={(e) => onUpload(e.target.files?.[0])} style={{ display: "none" }} />
                                </label>
                            </div>
                            ...
                            <div className="sidebarBlock">
                                ...
                                <div className="mapRow">
                                    <div className="mapLeft">
                                        <div className="mapName">라벨 <span className="req">(필수)</span></div>
                                    </div>
                                    <div className="mapRight">
                                        <Select value={labelCol} onChange={setLabelCol} options={[{ value: "", label: "선택…" }, ...columns.map((c) => ({ value: c, label: c }))]} />
                                    </div>
                                </div>
                                <div className="mapRow">
                                    <div className="mapLeft">
                                    ...
                                    <div className="mapRight">
                                        <div className="rowGap">
                                            <Select value={valueStart} onChange={setValueStart} options={timeCandidates.map((c) => ({ value: c, label: c }))} />
                                            <span className="mutedSmall">→</span>
                                            <Select value={valueEnd} onChange={setValueEnd} options={timeCandidates.map((c) => ({ value: c, label: c }))} />
                                        </div>
                                        {!timeCandidates.length ? (
                                            <div className="warn">⚠️ 연도 컬럼을 못 찾았어. 컬럼명이 1960, 2005 같은 형태인지 확인해줘.</div>
                                        ) : null}
                                    </div>
                                </div>
                                <div className="mapRow">
                                    ...
                                    <div className="mapRight">
                                        <Select value={categoryCol} onChange={setCategoryCol} options={[{ value: "", label: "(없음)" }, ...columns.map((c) => ({ value: c, label: c }))]} />
                                    </div>
                                </div>
                                <div className="mapRow">
                                    <div className="mapLeft">
                                ...
                                <button className="primaryBtn" type="button" onClick={() => setTab("preview")} disabled={!hasReady} title={!hasReady ? "라벨 + 연도(값) 컬럼이 필요해" : ""}>
                                    미리보기로 이동
                                </button>
                            </div>
                        </>
                        ...
                        <>
                            <Section title="설정">
                                <FieldRow label="표시 막대 수">
                                    <div className="rowGap">
                                        <Select value={String(topN)} onChange={(v) => setTopN(Number(v))} options={[8, 10, 12, 14, 16, 20].map((n) => ({ value: String(n), label: String(n) }))} />
                            ...
                            <Section title="색상">
                                <FieldRow label="Color mode">
                                    <Select value={colorMode} onChange={setColorMode} options={[{ value: "category", label: "By category" }, { value: "label", label: "By bar" }, { value: "single", label: "Single" }]} />
                                </FieldRow>
                                {colorMode !== "single" ? (
                                    <>
                            ...
                            <Section title="재생/타임라인">
                                ...
                                <div className="rowGap" style={{ marginTop: 8 }}>
                                    ...
                                    <button className="ghostBtn" type="button" onClick={() => { setPlaying(false); tRef.current = 0; setT(0); }}>
                                        처음으로
                                    </button>
                                </div>
                            </Section>
                            ...
                            <Section title="축/그리드">
                                ...
                                <FieldRow label="눈금 개수">
                                    <Select value={String(tickCount)} onChange={(v) => setTickCount(Number(v))} options={[2, 3, 4, 5].map((n) => ({ value: String(n), label: String(n) }))} />
                                </FieldRow>
